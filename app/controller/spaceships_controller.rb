...
# new
  ...
  post '/spaceships' do
    if logged_in?
      @spaceship = Spaceship.create(user_id: current_user.id, name: params[:name], ship_type: params[:ship_type], speed: params[:speed], cargo: params[:cargo], crew_member: params[:crew_member])
      erb :'spaceships/show.html'
    else
      erb :'error'
    end  
